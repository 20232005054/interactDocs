...
async def create_core_info(document_id: uuid.UUID, core_info: CoreInfoCreate, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def get_core_info(core_info_id: uuid.UUID, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def get_core_info_by_document(document_id: uuid.UUID, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def update_core_info(core_info_id: uuid.UUID, core_info: CoreInfoUpdate, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def delete_core_info(core_info_id: uuid.UUID, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    existing = await CoreInfoService.get_core_info_by_id(db, core_info_id)
    if not existing:
        raise HTTPException(status_code=404, detail="核心信息不存在")
...
async def update_core_info_order(core_info_id: uuid.UUID, order_update: CoreInfoOrderUpdate, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    existing = await CoreInfoService.get_core_info_by_id(db, core_info_id)
    if not existing:
        raise HTTPException(status_code=404, detail="核心信息不存在")
...
async def lock_core_info(core_info_id: uuid.UUID, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    existing = await CoreInfoService.get_core_info_by_id(db, core_info_id)
    if not existing:
        raise HTTPException(status_code=404, detail="核心信息不存在")
...
async def unlock_core_info(core_info_id: uuid.UUID, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    existing = await CoreInfoService.get_core_info_by_id(db, core_info_id)
    if not existing:
        raise HTTPException(status_code=404, detail="核心信息不存在")
...
async def reorder_core_info(document_id: uuid.UUID, data: CoreInfoReorder, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    try:
