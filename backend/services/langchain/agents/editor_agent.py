...
class DocumentEditorAgent:
    ...
    async def _call_model(self, state: AgentState) -> Dict:
        ...
        # 添加系统提示
        system_message = {"role": "system", "content": EDITOR_SYSTEM_PROMPT}
    ...
    async def analyze_document(self) -> Dict[str, Any]:
        ...
        user_message = HumanMessage(
            content=f"请分析文档 {self.document_id} 的结构和内容，"
                    "识别缺失或不完整的部分，并生成改进建议。"
        )
        ...
        try:
            ...
            # 提取建议
            suggestions = self._extract_suggestions_from_messages(messages)
    ...
    def _extract_suggestions_from_messages(self, messages: List[BaseMessage]) -> List[Dict]:
        """
        从消息列表中提取建议
        ...
        for msg in messages:
            if isinstance(msg, ToolMessage):
                # 检查是否是建议工具的返回
                if "[SUGGESTION]" in msg.content:
                    try:
                        suggestion_str = msg.content.split("[SUGGESTION]")[1].strip()
